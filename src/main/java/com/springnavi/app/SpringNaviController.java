...
public class SpringNaviController {
	...
	public void SpringNaviConstruct() {
		...
		com.springnavi.app.GlobalValueables.producer = new KafkaProducer<>(properties, new StringSerializer(), new StringSerializer());
		...
		properties.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, "localhost:9092");
		com.springnavi.app.GlobalValueables.producer10 = new KafkaProducer<>(properties, new StringSerializer(), new StringSerializer());
	...
	/*
	 * ログイン処理時のPWとID（CIFの番号ですが）を確認する処理
	 * ログインとか言ってるんですけど、セッション管理とかしてません。すみません。
	 */
	...
	public ResponseEntity<?> furikomiExec(@Validated @RequestBody FurikomiExecMessage furikomiExecMessage) {
		furikomiExecMessage = furikomiService.furikomiQueuing(furikomiExecMessage, com.springnavi.app.GlobalValueables.topicname);
	...
	public ResponseEntity<?> furikomiExec10(@Validated @RequestBody FurikomiExecMessage furikomiExecMessage) {
		furikomiExecMessage = furikomiService.furikomiQueuing(furikomiExecMessage, com.springnavi.app.GlobalValueables.topicname10);
		return new ResponseEntity<>(furikomiExecMessage, HttpStatus.OK);
	}
	...
	@PostMapping
	public ResponseEntity<?> asyncFurikomiStop(@Validated @RequestBody AsyncFurikomiControllMessage asyncFurikomiControllMessage) {
	...
	@PostMapping
	public ResponseEntity<?>  asyncFurikomiStart(@RequestBody AsyncFurikomiControllMessage asyncFurikomiControllMessage) throws Exception {
	...
	@PostMapping
	public ResponseEntity<?> asyncFurikomiStop10(@Validated @RequestBody AsyncFurikomiControllMessage asyncFurikomiControllMessage) {
	...
	@PostMapping
	public ResponseEntity<?>  asyncFurikomiStart10(@RequestBody AsyncFurikomiControllMessage asyncFurikomiControllMessage) throws Exception {
	...
	/*
	 * 疎通用テストメソッド
	 * テストコード書けよ、だとは思いますがお許しください
	 */
	@RequestMapping(value = "/test")
	public void test() {
		if (furikomiService.test()) {System.out.println("----- here test is succeeded-----");} 
